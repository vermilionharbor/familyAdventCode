...
for (let row=1; row<height-1; row++) {
    ...
    for (let b of Object.keys(beamMap)) {
        ...
        if (val === ".") {
            // beam continues
            if (nextMap[col] === undefined) {
                nextMap[col] = beamMap[col];
            } else {
                nextMap[col] = nextMap[col] + beamMap[col];
            }
        } else if (val === "^") {
            numSplits++;
            ...
            if (col-1 >= 0) {
                if (nextMap[col-1] === undefined) {
                    nextMap[col-1] = beamMap[col];
                }  else {
                    nextMap[col-1] = nextMap[col-1] + beamMap[col];
                }
            }
            if (col+1 < width) {
                if (nextMap[col +1] === undefined) {
    ...
    console.log(`row ${row} ${JSON.stringify(nextMap)}`);
    beamMap = nextMap;
}
...
console.log(beamMap);
let numTimeLines = 0;
for (let [k, v] of Object.entries(beamMap)) {
    numTimeLines += v;
}

console.log(`num time lines ${numTimeLines}`);